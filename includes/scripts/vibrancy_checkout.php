...
$landing_page = new C_landing_pages('vibrancy-landing');
  ...
  //  echo $orderid;exit;
    ...
    if(intval($orderid) > 0){
       ...
       // exit;
        ...
        $email_name = 'Vibrancy Order Success';
    ...
    util_redirect('vibrancysuccess?order='.$orderid);
...
while($p_res && $product = db_fetch_assoc($p_res)) {
    ...
    if ($product['stock_item_id'] == 73) {
        $product1=$product;
    }
    if ($product['stock_item_id'] == 74) {
        $product2=$product;
    }
    if ($product['stock_item_id'] == 75) {
        $product3=$product;
    }if ($product['stock_item_id'] == 76) {
        $product4=$product;
    }
}
